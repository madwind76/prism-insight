...
async def write_report_section(data_key: str, writer_agent: Agent, collected_data: dict,
    ...
    section_report = await llm.generate_str(
        message=f"""{company_name}({company_code})의 {data_key} 분석 보고서를 작성해주세요.
                    ...
                    보고서 작성시 제목을 작성하기 전에, 제일 첫 부분에 개행문자("\n")를 이용해서 문단을 구분하고 시작해주세요.
...
def clean_markdown(text: str) -> str:
    """
    LLM 출력에서 '#' 마크다운 제목 형식을 올바르게 수정하는 함수

    Args:
        text (str): 정리가 필요한 마크다운 텍스트
    ...
    for line in text.split('\n'):
        ...
        # 라인 중간에 #이 있는 경우만 처리
        if '#' in line:
            before_hash = line[:line.find('#')].rstrip()
            after_hash = line[line.find('#'):].lstrip()
            if before_hash:
                result.append(before_hash)
            result.append(after_hash)
        else:
            result.append(line)
