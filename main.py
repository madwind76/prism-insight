...
async def analyze_stock(company_code: str = "000660", company_name: str = "SK하이닉스", reference_date: str = None):
    ...
    async with app.run() as parallel_app:
        ...
        report_writers = {
            "technical_analysis": Agent(
                ...
                instruction=f"""당신은 주식 기술적 분석 전문가입니다. 주어진 종목의 주가 데이터, 거래량 데이터를 종합적으로 분석하여 기술적 분석 보고서를 작성해야 합니다.
                                ...
                                2. 거래량 및 투자자 동향 분석 - 거래량 패턴, 주요 투자자별 매매 동향
            ...
            "news_analysis": Agent(
                ...
                instruction=f"""당신은 기업 뉴스 분석 전문가입니다. 주어진 기업 관련 최근 뉴스와 이벤트를 분석하여 뉴스 트렌드 분석 보고서를 작성해야 합니다.

                                ## 수집해야 할 데이터
                                1. 기업 및 업종/산업 관련 뉴스: search tool을 사용하여 "{company_name} 업종 산업" 관련 최신 뉴스 검색 (최근 1-3개월 내)
                
                                ## 분석 요소
                                1. 주요 뉴스 요약 (기업 실적, 신제품, 인사, 이벤트 등 주요 소식)
                                2. 관련 업종 동향 정보 (뉴스에서 언급된 경우)
                                3. 향후 주목할만한 이벤트 (공시 예정, 실적 발표 등)
                                
                                ## 보고서 구성
                                1. 핵심 뉴스 요약 - 최근 주요 소식 요약
                                2. 간략한 영향 분석 - 뉴스가 기업에 미칠 수 있는 영향
                                3. 향후 주시점 - 언급된 향후 이벤트
                                ...
                                - 보고서 시작 시 개행문자 2번 삽입(\\n\\n)
                                - 제목: "# 3. 최근 주요 뉴스 요약"
                                - 주요 뉴스는 날짜와 출처를 포함하여 불릿 포인트로 요약
                
                                ## 주의사항
                                - search 도구는 단 한 번만 사용하고, numResults 파라미터를 5로 설정
                                - 검색 예시: search({{"query": "{company_name} 최근 소식", "numResults": 5}})
                                - 실제 뉴스에서 확인된 내용만 포함
                                - 뉴스가 부족한 경우 명확히 언급
            ...
            "investment_strategy": Agent(
                ...
                ## 분석 통합 요소
                1. 기술적 분석 요약 - 주가 추세, 주요 지지/저항선, 거래량 패턴
                2. 기업 기본 정보 요약 - 핵심 사업 모델, 경쟁력, 성장 동력
                3. 재무 분석 요약 - 수익성, 밸류에이션, 재무 건전성
        ...
        base_sections = ["technical_analysis", "company_status", "company_overview", "news_analysis"]
        ...
        # 최종 보고서 조합
        section_order = ["technical_analysis", "company_status", "company_overview", "news_analysis", "investment_strategy"]
...
if __name__ == "__main__":
    ...
    # 결과 저장
    with open(f"문배철강_분석보고서_{datetime.now().strftime('%Y%m%d')}_claude_3_7.md", "w", encoding="utf-8") as f:
        f.write(result)
